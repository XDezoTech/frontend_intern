...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-40 transition-all duration-500 ${bg}`} role="banner">
      <nav className="max-w-6xl mx-auto px-4 md:px-8 flex items-center justify-between h-16" aria-label="Main navigation">
        {/* Logo */}
        <Link to="/" className="flex items-center gap-2.5 group" aria-label="Himalaya Trails Home">
          <span className="text-saffron text-2xl">⛰</span>
          <span className="font-heading text-white text-base leading-none">Himalaya Trails</span>
        </Link>
        ...
        <Link
          to="/contact"
          className="hidden md:inline-flex items-center bg-crimson text-white text-xs tracking-widest uppercase px-5 py-2.5 hover:bg-crimson-light transition-colors font-medium min-h-[44px]"
        >
          Book a Trek
        </Link>
        ...
        {open && (
          ...
          >
            <div className="px-6 py-5 space-y-1">
              ...
              <Link to="/contact" className="block mt-4 text-center bg-crimson text-white text-xs tracking-widest uppercase py-3 font-medium min-h-[44px] flex items-center justify-center">
                Book a Trek
              </Link>
            </div>
          </motion.div>
